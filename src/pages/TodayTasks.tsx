                ...
                {ptasks.map(task => (
                  <div key={task.id} className="flex items-center gap-4 px-5 py-4">
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="text-[13px] font-medium" style={{ color: '#e8e8e8' }}>{task.title}</p>
                      {task.description && (
                        <p className="text-[12px] mt-0.5 truncate" style={{ color: '#666' }}>{task.description}</p>
                      )}
                    </div>
                    <div className="flex items-center gap-2">
                      {task.status === 'em-progresso' && (
                        <span className="text-[10px] px-1.5 py-0.5 rounded" style={{ background: 'rgba(245,158,11,0.15)', color: '#f59e0b', fontFamily: '"DM Mono", monospace' }}>
                          Em progresso
                        </span>
                      )}
                      <span
